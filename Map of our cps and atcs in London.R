...
ATCs_sf <- ATCs_sf %>% mutate(summary = paste0("<b>", "Site number = ", as.character(Site_Number),
                                               "<br/>", "CP number = ", as.character(CP.Number),                                          
...
CP_B_labels <- CPs_B_roads$summary  %>% lapply(HTML)  
CP_CU_labels <- CPs_CU_roads$summary  %>% lapply(HTML)  

                                            
leaflet() %>% addProviderTiles(providers$OpenStreetMap) %>%   
  addCircles(data = ATCs_CU_roads, 
             weight = 5, 
             opacity = 0.8,
             color = 'blue',
             fillOpacity = 0.5,
